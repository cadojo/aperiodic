...
you need to write contents to a `.py` file, make that file available on `sys.path`, 
and then load that module (typically with an `import` statement). 
[The `sys.path` variable is where Python will look for the name `something` when you type 
`import something`. You can check this path yourself! Try `import sys; print(sys.path)` sometime.]{.aside}
...
variable in all of our Python modules.)[This rule also applies to packages and 
subpackages because they are also Python modules!]{.aside} This `__export__` variable 
