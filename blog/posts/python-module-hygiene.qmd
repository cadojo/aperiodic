...
image: "/images/module-hygiene.svg"
categories:
  - python
  - design-patterns
  - technical
...
and then load that module (typically with an `import` statement.) [The `sys.path` variable is where Python will look for the name `something` when you type `import something`. You can check this path yourself! Try `import sys; print(sys.path)` sometime.]{.aside}
